...
dnl extra required libs
  ...
  py_link_for_shared=`sed -n -e 's/^LINKFORSHARED=\(.*\)/\1/p' $py_makefile`

dnl now the actual libpython
  if test -e "${py_exec_prefix}/lib/python${PYTHON_VERSION}/config/libpython${PYTHON_VERSION}.a"; then
    PYTHON_LIBS="${py_link_for_shared} -L${py_exec_prefix}/lib/python${PYTHON_VERSION}/config -lpython${PYTHON_VERSION} $py_localmodlibs $py_basemodlibs $py_other_libs"
    AC_MSG_RESULT(found)
  else
    AC_MSG_RESULT(not found)
  fi
