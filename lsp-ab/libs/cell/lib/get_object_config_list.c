...
int get_object_config_list(_comm_header * comm, _path * path,
			    unsigned long base, _tag_data * tags, int debug)
{
  ...
  while (1)
  {
    ...
    {
      dprint(DEBUG_BUILD,
	     "Got good reply to Get Object Config List Command - %d\n",
	     buff->overall_len);
    ...
    for (x = 44; x < buff->overall_len; x += 4)
    {
      ...
      {
        int i;
        dprint (DEBUG_TRACE, "Ran out of tag space - get_object_config_list.c\n");
	...
	tags->tag[tags->count + count]->base =
	 (byte) buff->data[x] +
	  ((byte)buff->data[x + 1] * 0x100) +
	  ((byte)buff->data[x + 2] * 0x10000) + ((byte)buff->data[x + 3] * 0x1000000);
	tags->tag[tags->count + count]->id = 0;
	dprint (DEBUG_BUILD,"Retrieved BASE value: %08lX\n",tags->tag[tags->count + count]->base);
      }
      else
      {
	...
	tags->tag[tags->count + count]->id =
	  ...
	  ((byte)buff->data[x + 1] * 0x100) +
	  ((byte)buff->data[x + 2] * 0x10000) + ((byte)buff->data[x + 3] * 0x1000000);
	dprint (DEBUG_BUILD,"Retrieved ID value: %08lX\n",tags->tag[tags->count + count]->id);
      }
      count++;
    }
    tags->count += count;
  ...
  dprint(DEBUG_TRACE, "Exiting get_object_config_list.c\n");
  return 0;
}
