...
{
  unsigned int x, size, totalsize, start;
  ...
  dprint(DEBUG_TRACE,"Elem1 = %ld, Elem2 = %ld, Elem3 = %ld\n",elem[0], elem[1], elem[2]);
...
//buff = buff + ENCAPS_Header_Length;
    ...
    {
      dprint(DEBUG_BUILD,
	     "Got reply to Read Tag Command - [%d] - %s\n",
	     buff->overall_len, get_reply_status(buff, debug));
...
tag->datalen);
  ...
  tag->type = buff->data[44] + (buff->data[45] * 256);
  dprint (DEBUG_TRACE, "data type =  %04X\n",tag->type);
  start = 46;
  if (tag->type == 0x2a0)
  	start = 48;
  tag->data = malloc(buff->overall_len - start);
  if (tag->data == NULL)
  	{
  	CELLERROR(21,"Could not allocate memory for tag data");
  	if (ioi2 != NULL)
  		free (ioi2);
  	free (msgbuff);
  	free (buff);
  ...
  memcpy (tag->data, &buff->data[start], buff->overall_len - start);
  tag->datalen = buff->overall_len - start;
