...
int object_bit_modify_byte(_comm_header * comm, _path * path, _tag_detail * tag,
			byte andmask, byte ormask, unsigned long offset, int debug)
{
  unsigned long a, b;
  int x, size, totalsize=0;
  int ret=0;
  _tag_detail *basetag;
  _data_buffer *buff;
  _encaps_header *head;
  ...
  dprint(DEBUG_TRACE, "object_bit_modify_byte.c entered.\n");
  ioi2 = NULL;
  ...
  size = 0;
    ...
    msgbuff->data[msgbuff->len++] = OBJECT_RMW_BYTE;
    ioi(msgbuff, ioi1, ioi2, debug);
    msgbuff->data[msgbuff->len++] = (byte)(offset & 255);
    msgbuff->data[msgbuff->len++] = (byte)((offset >> 8) & 255);
    msgbuff->data[msgbuff->len++] = (byte)((offset >> 16) & 255);
    msgbuff->data[msgbuff->len++] = (byte)((offset >> 24) & 255);
    msgbuff->data[msgbuff->len++] = 1;
    msgbuff->data[msgbuff->len++] = 0;
    msgbuff->data[msgbuff->len++] = ormask;
    msgbuff->data[msgbuff->len++] = andmask;
