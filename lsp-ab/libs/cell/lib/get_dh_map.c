...
int get_dh_map(_comm_header * comm, _path * path, _dhplus_map *data,
		     int dhport, int debug)
{
  ...
  dprint(DEBUG_TRACE, "get_dh_map.c entered.\n");
  ...
  if (data == NULL)
  {
    CELLERROR(2,"dhplus struct not allocated");
    return -1;
  }
  ...
  {
    ret=-1;
    dprint(DEBUG_VALUES, "Get DH map command returned an error.\n");
    CELLERROR(9,"Get DH map command returned an error");
  }
  if (ret)
  {
    free(buff);
...
//buff = buff + ENCAPS_Header_Length;
  if (buff->overall_len > 44)
  {
    ...
    memcpy(data, buff->data + 44, buff->overall_len - 44);
  }
  if (debug != DEBUG_NIL)
  {
    dprint(DEBUG_VALUES, "Got good reply to Get DH Map Command - %d\n",
	   buff->overall_len);
  ...
  dprint(DEBUG_TRACE, "Exiting get_dh_map.c\n");
