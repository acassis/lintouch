		      ...
		      unsigned offset, unsigned count, int debug)
{
  ...
  buff->len = 0;
    ...
    if (tag->alias_topbase != 0)
      {
      ioi1->ioiclass = -1;
      ioi1->instance = (tag->base & 0xffff); //was alias_topbase
      }
    ioi1->member = -1;
    ioi1->point = -1;
    ioi1->attribute = -1;
	...
	return -1;
      ...
      if (tag->alias_linkid != 0)
		//ioi2->instance = tag->alias_id; //*** removed
		ioi2->instance = (tag->alias_topbase&0xffff) + 24; //*** added
      ioi2->member = -1;
      ioi2->point = -1;
      ioi2->attribute = -1;
      ioi2->tagname = NULL;
...
tag->datalen);
    ...
    {
    int junk = 0;
    if (((tag->alias_type & 0xf000) == 0x8000) && ((tag->type & 0xf000) != 
0x8000))
      junk = tag->linkid - tag->alias_linkid;
    memcpy(&tag->data[offset],
	     buff->data + 44 + junk, tag->size);
    }
    tag->datalen += size;
