...
int read_multi_object_value(_comm_header * comm, _path * path,
			    _tag_list * list, int debug)
{
  ...
  dprint(DEBUG_TRACE, "read_multi_object_value.c entered.\n");
  ioi2 = NULL;
  size = 0;
  offset = 0;
  totalsize = 0;
  ...
  if (list == NULL)
  {
    CELLERROR(2,"list structure not allocated");
    return -1;
  }
...
// while (1){
  ...
  msgbuff->data[msgbuff->len++] = MULTI_SERVICE;
  ioi1->ioiclass = ROUTER;
  ioi1->instance = 1;
  ioi1->member = -1;
  ioi1->point = -1;
  ioi1->attribute = -1;
  ...
  for (x=0;x<list->count;x++)
    {
    stuff = 2+(list->count * 2) + ioilist->len;
    junk = list->tag[x]->linkid;
//    if (list->tag[x]->alias_linkid != 0)
  //   junk = list->tag[x]->alias_linkid;
    totalsize = getsize(list->tag[x]);
    ioilist->data[ioilist->len++] = DATA_TYPE_CONFIG;
    ioi1->ioiclass = DATA_OBJECT;
...
//    junk = getsize(list->tag[x])/4;
    junk = 1;
    ioilist->data[ioilist->len++] = (byte)(junk & 255);
    ioilist->data[ioilist->len++] = (byte)((junk >> 8) & 255);
      
    msgbuff->data[msgbuff->len++] = (byte)(stuff & 255);
    msgbuff->data[msgbuff->len++] = (byte)(stuff / 256);
    }
  ...
  {
    ret = -1;
    dprint(DEBUG_VALUES, "Read Object Value command returned an error.\n");
  }
  if (ret)
  {
    free(ioi2);
...
//buff = buff + ENCAPS_Header_Length;
  if (debug != DEBUG_NIL)
  {
    dprint(DEBUG_BUILD, "Got good reply to Read Object Value Command - %d\n",
	   buff->overall_len);
  ...
  for (x = 0; x < list->count; x++)
    {
    if (buff->data[start] == 0xcc)
      {
      start += 2;
      if (buff->data[start] == 0)
        {
        start += 2;
        if (list->tag[x]->data != NULL)
          memcpy (list->tag[x]->data, buff->data + start, 4);
        }
      }
    else
      {
  ...
  free(sendbuff);
  free(ioi1);
  dprint(DEBUG_TRACE, "read_multi_object_value.c exited.\n");
  return x;
}
