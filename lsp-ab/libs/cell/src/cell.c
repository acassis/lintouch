...
int main(int argc, char **argv)
{
  ...
  int x, y, z, debug, program_count, quiet, result=0;
  ...
  dprint (DEBUG_TRACE,"CPU found in slot %d\n",rack->cpulocation);
  path->device1 = 1;
...
printf ("configtags count = %d, detailtags count = %d\n",configtags->count, detailtags->count);
  ...
  for (z = 0; z < y; z++)
  {
	...
	break;
      default:
	result = read_object_value(comm, path, detailtags->tag[z], debug);
      }
	...
	c1++;
      ...
      {
        if (result != 0)
          printf ("Read %s failed: code %d\n",detailtags->tag[z]->name,result);
        else
    printf
      ("%s[%ld,%ld,%ld]:\n  topbase: 0x%08lX  base: 0x%08lX  id: 0x%08lX  linkid: 0x%08lX\n  size: %4d  type:0x%04X  memory:0x%08lX\n  alias_linkid: 0x%08lX  alias_size: %4d\n  alias_base: 0x%08lX  alias_type: 0x%04X\n  alias_topbase: 0x%08lX  alias_id: 0x%08lX\n",
       detailtags->tag[z]->name, detailtags->tag[z]->arraysize1,
       detailtags->tag[z]->arraysize2, detailtags->tag[z]->arraysize3,
       detailtags->tag[z]->topbase, detailtags->tag[z]->base,
       detailtags->tag[z]->id, detailtags->tag[z]->linkid,
       detailtags->tag[z]->size, detailtags->tag[z]->type, detailtags->tag[z]->memory,
       detailtags->tag[z]->alias_linkid, detailtags->tag[z]->alias_size,
       detailtags->tag[z]->alias_base, detailtags->tag[z]->alias_type,
       detailtags->tag[z]->alias_topbase, detailtags->tag[z]->alias_id);
     printf ("---------------------------------------------\n");  
...
for (z=0; z<detailtags->count; z++)
	{
//	if (strncasecmp(detailtags->tag[z]->name,"zzz",3) == 0)
          {
